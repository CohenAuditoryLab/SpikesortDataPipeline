...
for /D %%f in (*) do (
  mkdir %dest%\%%f
  cd %dest%\%%f
  mkdir KiloSort WaveClus Metrics
  move %origin%\%%f %dest%\%%f\RawData
  :: Register TDT
  :: Create MATLAB file
  matlab -nodisplay -nosplash -nodesktop -r TDTtoKiloSort(%dest%\%%f\%%f.mat,%dest%\%%f\KiloSort)
  matlab -nodisplay -nosplash -nodesktop -r mat2waveclus(%dest%\%%f\%%f.mat,%dest%\%%f\WaveClus)
